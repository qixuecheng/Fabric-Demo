...
public class InvokeChainCode {
	...
	public InvokeChainCode(String[] args) throws CryptoException, InvalidArgumentException, NoSuchAlgorithmException,
		...
		this.client.setUserContext(sampleOrg.getPeerAdmin()); // Maybe a bug of 1.0.0beta, only peer admin can call
																// chaincode?
	}

	public InvokeChainCode() throws CryptoException, InvalidArgumentException, NoSuchAlgorithmException,
			NoSuchProviderException, InvalidKeySpecException, TransactionException, IOException {
		//this.args = args;
		this.client = clientHelper.getHFClient();
		this.channel = clientHelper.getChannel();
	...
	public void invoke() throws InvalidArgumentException, ProposalException, InvalidProtocolBufferException,
		...
		channel.sendTransaction(successful).thenApply(transactionEvent -> {
			//String transactionEvent.getChannelId();
			String txid = transactionEvent.getChannelId();
			Date dateTmp2 = new Date();
			long time = dateTmp2.getTime() - dateTmp1.getTime();
			System.out.println("cccc" + time);
			// if (transactionEvent.isValid()) {
			// logger.info("Successfully send transaction proposal to orderer. Transaction
			// ID: "
			...
			// }
			// chain.shutdown(true);
			// System.out.println(txid + "的用时:"+ time);
	...
	public void queryByRefNo() throws InvalidArgumentException, ProposalException {
		...
		// logger.error("fxxk"+args[0]+args[1]+args[2]);
